	...
	onAdd: function(map){
		...
		if(this._usingCanvas){
			...
			if(this.options.animate && this._path.animate){
                ...
                let animation = this._path.animate([{ strokeDashoffset: length }, { strokeDashoffset: 0 }, {strokeDashoffset: -length}], this.options.animate);
				// console.log(animation);
				animation.onfinish = () => {this.remove();}
				// killMe(animation.ready, this);
				// console.log("started");

				// if (this.options.hasBalls){
				// 	this._path.reverse();
				// }
			}
		}
